...
class Crypto:
    ...
    def data_transform(self, ids, days=None):
        ...
        # Calculate daily total market cap and weighted average price
        daily_market_data = (
            df.groupby('date')
            .apply(lambda x: pd.Series({
                'total_market_cap': x['market_cap'].sum(),
                'crypto_index': (x['price'] * x['market_cap']).sum() / x['market_cap'].sum()  # Weighted average price
            }))
            .reset_index()
        )

        self.df = daily_market_data
        return self.df
